...
export async function extractProductModels(file: File, sharedContext: Record<string, string> = {}): Promise<ExtractedProductModel[]> {
  ...
  return (parsed.models as RawModel[]).filter((model) => typeof model?.name === "string" && model.name.trim()).map((model) => ({
    ...
    specifications: Object.fromEntries((Array.isArray(model.specifications) ? model.specifications : []).filter((item) => typeof item?.key === "string" && typeof item?.value === "string").map((item) => [item.key.trim().slice(0, 80), item.value.trim().slice(0, 500)]).filter(([key, value]) => key && value)),
  }));
}
