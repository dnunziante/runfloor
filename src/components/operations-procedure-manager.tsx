...
import { CategoryManagementMenu, ProcedureManagementMenu } from "./procedure-library-management";
...
export function OperationsProcedureManager({
  ...
  const orderedCategories = [...categories].sort((left, right) => {
    if (left.sortOrder !== undefined && right.sortOrder !== undefined) return left.sortOrder - right.sortOrder || left.name.localeCompare(right.name);
