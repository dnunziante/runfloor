...
import { procedureCategoryStyles, ProcedureCategoryCard, ProcedureValueBanner } from "./procedure-library-visuals";
...
import { CategoryManagementMenu, ProcedureManagementMenu, ProcedureLibraryManagement } from "./procedure-library-management";
import type { LibrarySnapshot } from "@/lib/procedures/library";

type Template = { id: string; title: string; category: string; owner: string; summary: string; version: number };

export function PlatformProcedureTemplateLibrary({ templates, tenants, children, snapshot, initialCategory = null, initialQuery = "" }: { snapshot: LibrarySnapshot; templates: Template[]; tenants: Array<{ id: string; name: string }>; children: ReactNode; initialCategory?: string | null; initialQuery?: string }) {
  ...
  const availableCategories = snapshot.error ? Array.from(new Set([...procedureCategoryStyles.map(item => item.name),...templates.map(item => item.category)])).map(name => ({id:name,name,description:"",style_name:name})) : snapshot.categories.filter(item => !item.archived_at);
  ...
  return <section className={styles.library} aria-labelledby="procedure-library-heading">
    ...
    <ProcedureLibraryManagement scope="platform" snapshot={snapshot} canManage onChanged={command => {
      if (snapshot.categories.find(item => item.id === command.id)?.name !== category) return;
      if (command.action === "category_update") setCategory(command.name ?? null);
      if (command.action === "category_archive" || command.action === "category_delete") { setCategory(null); clearSelection(); }
    }}>
    ...
    <div className={styles.categories} aria-label="Procedure template categories">
      {availableCategories.map(item => <div className={layout.categoryGroup} key={item.id}><ProcedureCategoryCard name={item.name} description={item.description} styleName={item.style_name} count={templates.filter(template => template.category === item.name).length} selected={category === item.name} disabled={saving} onClick={() => { setCategory(category === item.name ? null : item.name); setQuery(""); clearSelection(); }} /><CategoryManagementMenu id={item.id}/></div>)}
    </div>
