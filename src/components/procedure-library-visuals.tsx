...
export function ProcedureCategoryCard({ name, count, noun = "templates", selected, disabled, onClick, description: customDescription, styleName }: { name: string; count: number; noun?: string; selected: boolean; disabled?: boolean; onClick: () => void; description?: string; styleName?: string }) {
const { description: defaultDescription, icon: Icon, color, tint } = procedureCategoryStyle(styleName || name);
