...
export function OperationsScheduleManager({ initialSchedules = [], initialProcedures = [], persistence = "demo", initialError = "", canManage = true, isRv = false }: { initialSchedules?: OperationsScheduleRecord[]; initialProcedures?: OperationsProcedureRecord[]; persistence?: OperationsPersistence; initialError?: string; canManage?: boolean; isRv?: boolean }) {
  ...
  return <div className={`rs-workspace${isRv ? " rv-rs-workspace" : ""}`}>
    ...
    <div className="rs-layout"><aside className="rs-card rs-builder">{isRv && <header><h2>Create a New Recurring Schedule</h2><p>Turn your best processes into consistent habits.</p></header>}<span className="rs-chip">{persistence === "supabase" ? "Organization schedules" : "Local prototype"}</span>{!isRv && <><h2>Create a new schedule</h2><p>Choose a published procedure to define the checklist steps.</p></>}{procedures.length ? <form onSubmit={createSchedule}><label><span>Procedure *</span><select value={procedureId} onChange={(event) => setProcedureId(event.target.value)}>{procedures.map((item) => <option value={item.id} key={item.id}>{item.title}</option>)}</select></label><Link href="/operations/procedures">View procedure <ArrowRight/></Link><div><label><span>Frequency *</span><select value={frequency} onChange={(event) => setFrequency(event.target.value as OperationsScheduleRecord["frequency"])}><option>Daily</option><option>Weekly</option><option>Monthly</option></select></label><label><span>Location *</span><select value={location} onChange={(event) => setLocation(event.target.value)}><option>Charleston</option><option>Summerville</option><option>All locations</option></select></label></div><div className="rs-owner-fields"><label><span>Owner *</span><input required value={owner} onChange={(event) => setOwner(event.target.value)} placeholder="Person or team"/></label><label><span>Start date *</span><input required type="date" value={nextRunDate} onChange={(event) => setNextRunDate(event.target.value)}/></label></div><details><summary>Additional options</summary><p>Schedules generate from the latest published version of the selected procedure.</p></details><button type="submit"><Plus/> Create Schedule</button></form> : <div className="operations-schedule-empty"><AlertTriangle size={22}/><strong>No published procedures</strong><p>Publish a procedure before creating a recurring schedule.</p><Link className="btn btn-secondary" href="/operations/procedures">Manage procedures</Link></div>}</aside>
      <div className="rs-main"><section className="rs-card rs-upcoming"><header><h2>Upcoming scheduled work</h2><Link href="/operations/calendar">View all <ArrowRight/></Link></header>{upcoming.length ? <div className="rs-table-wrap"><table><thead><tr><th>Due date</th><th>Procedure</th><th>Location</th><th>Owner</th><th>Status</th><th>Actions</th></tr></thead><tbody>{upcoming.slice(0,6).map((schedule) => <tr key={schedule.id}><td>{formatOperationsDate(schedule.nextRunDate)}</td><td><strong>{schedule.procedureTitle}</strong><small>{schedule.frequency}</small></td><td>{schedule.location}</td><td>{schedule.owner}</td><td><span>● Scheduled</span></td><td><button title="Generate checklist now" onClick={() => generateChecklist(schedule)}><Play/></button><button title="Pause schedule" onClick={() => toggleStatus(schedule.id)}><CirclePause/></button></td></tr>)}</tbody></table></div> : <div className="rs-empty"><CalendarClock/><strong>No upcoming work</strong><p>Create or resume a schedule to see its next run.</p></div>}</section>
        <div className="rs-bottom"><section className="rs-card rs-templates"><header><div><FolderOpen/><h2>{isRv ? "Schedule Templates" : "Schedule templates"}</h2></div><Link href="/operations/procedures">View all <ArrowRight/></Link></header>{procedures.slice(0,4).map((procedure) => <button key={procedure.id} onClick={() => setProcedureId(procedure.id)}><span><strong>{procedure.title}</strong><small>{procedure.category} · {procedure.steps.length} steps</small></span><b>Use template</b></button>)}<Link href="/operations/procedures">Browse all procedures <ArrowRight/></Link></section><div className="rs-side"><aside className="rs-card rs-tip"><Lightbulb/><div><h2>{isRv ? "Pro Tip" : "Pro tip"}</h2><strong>Start with templates</strong><p>Use published procedures to ensure consistency across all locations.</p><Link href="/operations/procedures">Browse templates <ArrowRight/></Link></div></aside><blockquote>{isRv ? "Small Steps. Big Journeys." : "“Consistent execution creates exceptional results.”"}<small>{isRv ? "Keep every team moving forward." : "— RunFloor"}</small></blockquote></div></div>
      </div>
    </div>
