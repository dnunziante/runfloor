...
import { reactivateUserInWorkspace, removeUserFromWorkspace, updateUserCredentials, type ReactivateUserState, type RemoveUserState, type UpdateCredentialsState } from "@/app/admin/users/actions";
...
const initialReactivateState: ReactivateUserState = { error: "", success: "" };
...
function ReactivateUserForm({ membershipId, currentLocationId, locations }: { membershipId: string; currentLocationId: string | null; locations: Location[] }) {
  const [state, action, pending] = useActionState(reactivateUserInWorkspace, initialReactivateState);
  return <form action={action} className="user-access-form"><input type="hidden" name="membershipId" value={membershipId}/>{currentLocationId ? <input type="hidden" name="locationId" value={currentLocationId}/> : <select className="input" name="locationId" defaultValue="" aria-label="Location for reactivation" required><option value="" disabled>Choose location</option>{locations.map((location) => <option value={location.id} key={location.id}>{location.name}</option>)}</select>}<button className="btn btn-secondary" type="submit" disabled={pending}>{pending ? "Reactivating…" : "Reactivate"}</button>{state.error && <span className="form-error" role="alert">{state.error}</span>}{state.success && <span className="form-success" role="status">{state.success}</span>}</form>;
}
