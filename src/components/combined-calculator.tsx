...
export function CombinedCalculator({ vehicles, accessories, warranties, locations, isRv = false }: { vehicles: ProductDTO[]; accessories: ProductDTO[]; warranties: ProductDTO[]; locations: OrganizationLocation[]; isRv?: boolean }) {
  ...
  return <div className={`cq ${isRv ? "rv-cq" : ""}`}>
    ...
    <section className="cq-hero"><div><span>Quote &amp; financing</span><h1>{isRv ? <>Turn Dreams Into<br/><em>Road Trips.</em></> : <>Turn Interest Into Ownership.<br/><em>Premium Style. Real Performance.</em></>}</h1><p>Calculate total price, estimate a monthly payment, and build a professional quote in one place.</p><div><b><Calculator/> Real payments<small>Live calculations</small></b><b><FileDown/> All fees included<small>{isRv ? "Transparent pricing" : "Delivered price"}</small></b><b><Share2/> Ready to share<small>Print or email</small></b></div></div>{isRv && <blockquote>Adventure.<br/>Finances.<br/>Family.<br/>All in One Place.</blockquote>}</section>
    <div className="cq-layout">
      <div className="cq-left">
        <section className="cq-card cq-product-card"><header><span>1</span><div><h2>Select product</h2><p>Choose a vehicle and configuration.</p></div></header><div className="cq-fields"><label>Vehicle<select value={vehicleId} onChange={(e) => selectVehicle(e.target.value)}><option value="manual">Custom price</option>{vehicles.map((vehicle) => <option value={vehicle.id} key={vehicle.id}>{vehicle.name} · {vehicle.model || "Standard"} · {money(vehicle.price)}</option>)}</select></label><label>Location<select value={locationId} onChange={(e) => chooseLocation(e.target.value)}><option value="manual">Manual fees</option>{locations.map((location) => <option value={location.id} key={location.id}>{location.name}</option>)}</select></label></div><Link href="/products">View full product details →</Link>{isRv && <div className="rv-cq-product-art"><strong>Explore. Compare. Customize.</strong><small>Your next adventure starts here.</small></div>}</section>
        <section className="cq-card"><header><span>2</span><div><h2>Customize your build</h2><p>Add published accessories and warranties.</p></div><Link href="/products">View product catalog →</Link></header>{isRv && <div className="rv-cq-categories"><span>Exterior &amp; Towing<small>Hitches, racks, awnings</small></span><span>Interior &amp; Comfort<small>Appliances, furniture, tech</small></span><span>Off-Grid &amp; Power<small>Solar, batteries, generators</small></span><span>Protection<small>Warranties, service plans</small></span></div>}<div className="cq-addons">{accessories.length ? accessories.map((item) => <label key={item.id}><input type="checkbox" checked={accessoryIds.includes(item.id)} onChange={() => toggleProduct(item.id, "accessory")}/><span>{item.name}<small>{item.model}</small></span><strong>{money(item.price)}</strong></label>) : <p>No published accessories are available.</p>}</div><details><summary>Extended warranties ({warrantyIds.length} selected)</summary><div className="cq-addons">{warranties.length ? warranties.map((item) => <label key={item.id}><input type="checkbox" checked={warrantyIds.includes(item.id)} onChange={() => toggleProduct(item.id, "warranty")}/><span>{item.name}<small>{item.model}</small></span><strong>{money(item.price)}</strong></label>) : <p>No published warranties are available.</p>}</div></details></section>
        <section className="cq-card cq-trade-card"><header><span>3</span><div><h2>Trade-in <small>(optional)</small></h2><p>Apply a trade value to this deal.</p></div></header><div className="cq-fields"><label>Make / Model / Year<input value={tradeDescription} onChange={(e) => setTradeDescription(e.target.value)} placeholder={isRv ? "e.g. Grand Design 2022" : "e.g. Club Car 2022"}/></label><label>Trade-in value<input type="number" min="0" value={quote.tradeIn} onChange={(e) => update("tradeIn", Number(e.target.value))}/></label></div>{isRv && <div className="rv-cq-trade"><strong>Trade In. Upgrade.<br/>Hit the Road Sooner.</strong><small>Get more for your trade and put it toward your next adventure.</small></div>}</section>
      </div>
