...
export function ProductEditor({ product, families, organizationId, isGolfCart = false }: { product: ProductDTO; families: ProductFamilyDTO[]; organizationId: string; isGolfCart?: boolean }) {
  const newImagesRef = useRef<NewImage[]>([]);
  const [state, setState] = useState(initialState);
  const [saving, setSaving] = useState(false);
  const [keptPaths, setKeptPaths] = useState(product.imagePaths);
  const [newImages, setNewImages] = useState<NewImage[]>([]);
  const knownSpecificationKeys = new Set(golfCartSpecificationSections.flatMap((section) => section.fields.map((field) => field.key)).concat(["series"]));
  ...
  return <form className="card form-stack product-edit-form" onSubmit={submit}>
    ...
    {isGolfCart && golfCartSpecificationSections.map((section) => <details key={section.title} open={section.fields.some((field) => Boolean(product.specifications?.[field.key]))}><summary><strong>{section.title}</strong></summary><div className="grid grid-2" style={{marginTop:14}}>{section.fields.map((field) => <label key={field.key}><span className="label">{field.label}</span>{field.multiline ? <textarea className="input" name={`spec.${field.key}`} rows={3} defaultValue={product.specifications?.[field.key] || ""}/> : <input className="input" name={`spec.${field.key}`} defaultValue={product.specifications?.[field.key] || ""}/>}</label>)}</div></details>)}
    {isGolfCart && additionalSpecifications.length > 0 && <details open><summary><strong>Additional Specifications</strong></summary><div className="grid grid-2" style={{marginTop:14}}>{additionalSpecifications.map(([key, value]) => <label key={key}><span className="label">{key}</span><input className="input" name={`spec.${key}`} defaultValue={value}/></label>)}</div></details>}
