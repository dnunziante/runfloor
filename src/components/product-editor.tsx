...
const specificationKind = (field: unknown) => (field as { kind?: "number" | "boolean" }).kind;
...
export function ProductEditor({ product, families, organizationId, isGolfCart = false, isRv = false }: { product: ProductDTO; families: ProductFamilyDTO[]; organizationId: string; isGolfCart?: boolean; isRv?: boolean }) {
  ...
  return <form className="card form-stack product-edit-form" data-industry={isRv ? "rv" : isGolfCart ? "golf-cart" : "generic"} onSubmit={submit}>
    ...
    {(isGolfCart || isRv) && specificationSections.map((section) => <details key={section.title} open={section.fields.some((field) => product.specifications?.[field.key] !== null && product.specifications?.[field.key] !== undefined && product.specifications?.[field.key] !== "")}><summary><strong>{section.title}</strong></summary><div className="grid grid-2" style={{marginTop:14}}>{section.fields.map((field) => <label key={field.key}><span className="label">{field.label}</span>{specificationKind(field) ? <input type="hidden" name={`specType.${field.key}`} value={specificationKind(field)}/> : null}{specificationKind(field) === "boolean" ? <select className="input" name={`spec.${field.key}`} defaultValue={product.specifications?.[field.key] === null || product.specifications?.[field.key] === undefined ? "" : String(product.specifications[field.key])}><option value="">Not provided</option><option value="true">Yes</option><option value="false">No</option></select> : field.multiline ? <textarea className="input" name={`spec.${field.key}`} rows={3} defaultValue={String(product.specifications?.[field.key] ?? "")}/> : <input className="input" type={specificationKind(field) === "number" ? "number" : "text"} step={specificationKind(field) === "number" ? "any" : undefined} name={`spec.${field.key}`} defaultValue={String(product.specifications?.[field.key] ?? "")}/>}</label>)}</div></details>)}
    {(isGolfCart || isRv) && additionalSpecifications.length > 0 && <details open><summary><strong>Additional Specifications</strong></summary><div className="grid grid-2" style={{marginTop:14}}>{additionalSpecifications.map(([key, value]) => <label key={key}><span className="label">{key}</span><input type="hidden" name={`specType.${key}`} value={typeof value}/><input className="input" name={`spec.${key}`} defaultValue={String(value ?? "")}/></label>)}</div></details>}
