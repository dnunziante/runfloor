...
export function AdminProductForm({ families, initialFamilyId = "", isRv = false, competitorOnly = false, initialBrand = "" }: { families: ProductFamilyDTO[]; initialFamilyId?: string; isRv?: boolean; competitorOnly?: boolean; initialBrand?: string }) {
  ...
  return <form ref={formRef} className={`card form-stack ${competitorOnly ? "competitor-entry-form" : ""}`} onSubmit={handleSubmit}>
    <div><h2>{competitorOnly ? "Add competitor product" : "Add product"}</h2><p style={{fontSize:12,marginBottom:0}}>New products and images belong only to the active organization.</p></div>
