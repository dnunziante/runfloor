...
export function AdminProductForm({ families, initialFamilyId = "", isRv = false, competitorOnly = false, initialBrand = "" }: { families: ProductFamilyDTO[]; initialFamilyId?: string; isRv?: boolean; competitorOnly?: boolean; initialBrand?: string }) {
  ...
  const [createdFamilies, setCreatedFamilies] = useState<ProductFamilyDTO[]>([]);
  const availableFamilies = [...families, ...createdFamilies.filter((item) => !families.some((family) => family.id === item.id))];
  const [productType, setProductType] = useState<"our_product" | "competitor_product">(competitorOnly ? "competitor_product" : "our_product");
  ...
  async function addFamily() {
    ...
    setState(initialState);
    const result = await createProductFamily(newFamilyName, competitorOnly);
    if (result.error || !result.familyId) {
      setState({ error: result.error || "The product category could not be created.", success: "" });
      setCreatingFamily(false);
      return;
    }
    setCreatedFamilies((current) => [...current, { id: result.familyId!, name: newFamilyName.trim(), slug: "", description: "", imageUrl: null, imagePath: null, productCount: 0 }]);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!competitorOnly) setSelectedFamilyId("");
  ...
  return <form id={isRv && !competitorOnly ? "add-product" : undefined} ref={formRef} className={`card form-stack ${competitorOnly ? "competitor-entry-form" : ""} ${isRv ? "rv-admin-product-form" : ""}`} onSubmit={handleSubmit}>
    <div><h2>{competitorOnly ? "Add competitor product" : isRv ? "Add / Edit Product" : "Add product"}</h2><p style={{fontSize:12,marginBottom:0}}>{isRv ? "Add a new RV model to this workspace catalog." : "New products and images belong only to the active organization."}</p></div>
    {isRv && !competitorOnly ? <div className="rv-product-form-tabs" aria-label="Product form sections"><span className="active">Basic Info</span><span>Media</span><span>Specifications</span><span>Pricing</span><span>More</span></div> : null}
    <div className="grid grid-2"><label><span className="label">Product Type</span><select className="input" name="productType" value={productType} onChange={(event) => setProductType(event.target.value as "our_product" | "competitor_product")}>{!competitorOnly && <option value="our_product">Our Product</option>}<option value="competitor_product">Competitor Product</option></select></label>{isRv ? <RvCategoryField/> : <label><span className="label">Product category</span><select className="input" name="productCategory" defaultValue={productType === "competitor_product" ? "Competitor Vehicles" : "Golf Cart / LSV"}><option>Golf Cart / LSV</option><option>Accessories</option><option>Warranty</option><option>Competitor Vehicles</option></select></label>}</div>
    {productType === "our_product" ? <div><label className="label" htmlFor="familyId">Catalog</label><select className="input" id="familyId" name="familyId" required value={selectedFamilyId} onChange={(event) => setSelectedFamilyId(event.target.value)}><option value="" disabled>Choose a catalog</option>{families.map((family) => <option value={family.id} key={family.id}>{family.name}</option>)}</select><div style={{display:"flex",gap:8,marginTop:8}}><input className="input" value={newFamilyName} onChange={(event) => setNewFamilyName(event.target.value)} placeholder="New catalog name" maxLength={120}/><button className="btn btn-ghost" type="button" disabled={creatingFamily || newFamilyName.trim().length < 2} onClick={() => void addFamily()}>{creatingFamily ? "Creating…" : "Create catalog"}</button></div><small className="field-help">Choose an existing catalog or create a tenant-only catalog.</small></div> : competitorOnly ? <div><label className="label" htmlFor="familyId">Product family</label><select className="input" id="familyId" name="familyId" required value={selectedFamilyId} onChange={(event) => { if (event.target.value === "__new__") { setAddingFamily(true); setState(initialState); } else { setSelectedFamilyId(event.target.value); setAddingFamily(false); } }}><option value="" disabled>Choose a brand family</option>{availableFamilies.map((family) => <option value={family.id} key={family.id}>{family.name}</option>)}<option value="__new__">Add new product family...</option></select>{addingFamily && <div className="form-stack" style={{marginTop:12}}><label><span className="label">New product family name</span><input className="input" value={newFamilyName} onChange={(event) => setNewFamilyName(event.target.value)} maxLength={120} placeholder="Enter brand name"/></label><div className="button-row"><button className="btn btn-secondary" type="button" disabled={creatingFamily || newFamilyName.trim().length < 2} onClick={() => void addFamily()}>{creatingFamily ? "Creating..." : "Create and select family"}</button><button className="btn btn-ghost" type="button" disabled={creatingFamily} onClick={() => setAddingFamily(false)}>Cancel</button></div></div>}<small className="field-help">This model and configuration will be saved inside the selected brand family.</small></div> : <input type="hidden" name="familyId" value=""/>}
    <div className={isAddOn ? "" : "grid grid-2"}><div><label className="label" htmlFor="name">{isRv ? "Product name" : `${itemLabel} name`}</label><input className="input" id="name" name="name" required placeholder={isRv ? "Solera" : isAddOn ? `${itemLabel} name` : "Nexus"}/></div>{!isAddOn && <div><label className="label" htmlFor="model">{isRv ? "Floorplan / Configuration" : "Configuration"}</label><input className="input" id="model" name="model" placeholder={isRv ? "1952F" : "4 Passenger Forward"}/></div>}</div>
    {competitorOnly && !isRv && <div className="grid grid-2"><label><span className="label">Brand</span><input className="input" name="brand" value={selectedFamily?.name || initialBrand} readOnly required/></label><label><span className="label">Model year</span><input className="input" name="modelYear" type="number" min="1900" max="2200"/></label></div>}
    {isRv && <div className="grid grid-2"><label><span className="label">Manufacturer</span><input className="input" name="manufacturer" placeholder="Forest River"/></label><label><span className="label">Brand</span><input className="input" name="brand" placeholder="Forest River"/></label><label><span className="label">Model Year</span><input className="input" name="modelYear" type="number" min="1900" max="2200" placeholder="2027"/></label><label><span className="label">Trim / Variant</span><input className="input" name="modelVariant" placeholder="Standard"/></label><label><span className="label">Product Line / Series</span><input className="input" name="spec.series" placeholder="Solera"/></label><label><span className="label">Sleeping Capacity</span><input className="input" name="spec.sleepingCapacity" placeholder="4"/></label></div>}
    ...
    <div className={competitorOnly ? "competitor-image-panel" : ""}>
      {competitorOnly && <div><h2>Upload product images</h2><p>Add images to this model. They are saved when you add the product.</p></div>}
      ...
      <label className="product-image-picker" htmlFor="product-images" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); selectImages(event.dataTransfer.files); }}><span className="product-image-prompt"><ImagePlus size={24}/><strong>Choose product images</strong><small>Automatically centered and scaled · JPG, PNG, or WebP · 5 MB each</small></span></label>
