...
const specificationKeys: Record<string, string> = { "shipping weight": "shippingWeight", "carrying capacity": "carryingCapacity", "hitch weight": "hitchWeight", length: "length", height: "height", "tire size": "tireSize", "fresh water": "freshWater", "gray water": "grayWater", "waste water": "wasteWater", "sleeping capacity": "sleepingCapacity", "number of propane tanks": "numberOfPropaneTanks", "propane tanks": "numberOfPropaneTanks", "lpg capacity": "lpgCapacity", "refrigerator size": "refrigeratorSize" };

function normalizedSpecifications(source: Record<string, string>) {
  return Object.fromEntries(Object.entries(source).map(([key, value]) => [specificationKeys[key.toLowerCase().trim()] || key.trim(), text(value, 500)]).filter(([key, value]) => key && value));
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (mode === "extract") {
    ...
    const context = { manufacturer: text(data.get("manufacturer")), modelVariant: text(data.get("modelVariant")), modelYear: text(data.get("modelYear")), productCategory: text(data.get("productCategory")), productType: text(data.get("productType")) };
    ...
    try {
      const extracted = await extractProductModels(file, context);
      ...
      const candidates = extracted.map((item) => {
        const product = { name: text(item.name, 180), model: text(item.model || item.name, 160), manufacturer: text(item.manufacturer || context.manufacturer), modelYear: item.modelYear || (Number(context.modelYear) || null), modelVariant: text(context.modelVariant), productCategory: text(item.category || context.productCategory, 120), productType: context.productType === "competitor_product" ? "competitor_product" : "our_product", description: text(item.description, 4000), specifications: normalizedSpecifications(item.specifications) };
      ...
      const approved = candidates.filter((candidate) => !candidate.duplicateId && candidate.name);
      if (approved.length) {
        const { error: insertError } = await db.from("industry_template_products").insert(approved.map((candidate) => ({ industry_template_id: templateId, source_import_id: importId, family_name: "Starter Products", name: candidate.name, model: candidate.model, model_year: candidate.modelYear, model_variant: candidate.modelVariant, manufacturer: candidate.manufacturer, product_category: candidate.productCategory, product_type: candidate.productType, description: candidate.description, specifications: candidate.specifications })));
        if (insertError) return NextResponse.json({ error: "The extracted starter products could not be saved." }, { status: 500 });
      }
      return NextResponse.json({ importId, candidates, created: approved.length, skippedDuplicates: candidates.length - approved.length });
    } catch (error) {
