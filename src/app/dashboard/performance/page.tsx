...
import { ArrowRight, BarChart3, Bot, Calculator, CheckCircle2, Clock3, GraduationCap, Mail, MessageSquare, Play, Star, Target } from "lucide-react";
...
export default async function Dashboard({ searchParams }: { searchParams: Promise<{view?:string;locationId?:string;employeeId?:string}> }) {
  ...
  return <AppShell title={data.team ? "Team Dashboard" : "Personal Dashboard"} industry={isRv ? "rv" : undefined}>
    <main className={`personal-command ${isRv ? "rv-performance" : ""}`}>
      <section className="personal-hero" aria-labelledby="performance-title">
        <div><span>{data.team ? "Team performance" : "Personal performance"}</span><h1 id="performance-title">{person ? <>Good morning, {person}! <i aria-hidden="true">👋</i></> : isRv && data.scope === "company" ? <>{viewer?.organizationName} — <em>Company Overview</em></> : data.title}</h1><p>{isRv && data.scope === "company" ? <>Performance across all {viewer?.organizationName} locations. People. Processes. Profit.</> : data.subtitle}</p></div>
        <blockquote>{isRv ? <>“More Families.<br/>Greater Adventures.”</> : <>“Small steps<br/>create big results.”<small>— RunFloor</small></>}</blockquote>
        {!isRv && <strong>Learn.<br/>Apply.<br/>Close.<br/><em>Grow.</em></strong>}
      </section>
      ...
      <section className="personal-lower">
        ...
        <article className="personal-card personal-resources"><header><h2>Top resources for you</h2><Link href="/knowledge-base">See all <ArrowRight/></Link></header><div>{resources.map(([Icon, label, copy, href]) => <Link href={href} key={label}><Icon/><strong>{label}</strong><small>{copy}</small><ArrowRight className="resource-arrow"/></Link>)}</div></article>

        <aside className="personal-cta"><Target/><h2>You&apos;re on <em>the right track.</em></h2><p>{isRv ? "Keep learning, keep growing, and help more families find their perfect RV." : "Keep learning, keep engaging, and keep creating value."}</p><Link href="/training">Explore more training <ArrowRight/></Link></aside>
      </section>
