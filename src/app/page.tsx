...
const pillars = [
  { icon: MessageSquareText, title: "Sales Assistant", copy: "Answers. Guidance. Consistent messaging.", href: "/assistant", items: ["Instant product answers","Objection handling","Email and text templates","Deal guidance"] },
  ...
  { icon: GraduationCap, title: "Sales Coach", copy: "Practice. Improve. Close more.", href: "/coach", items: ["Role-play training","Real objection scenarios","Personalized coaching","Team progress"] },
...
export default function Home() {
  return <main className="lp">
    <section className="lp-hero">
      ...
      <div className="lp-hero-grid"><div><p className="lp-kicker">The AI-powered dealership platform</p><h1>People.<br/>Processes.<br/><em>Profits.</em></h1><h2>Train your team. Standardize your operations.<br/>Sell more units.</h2><p>RunFloor gives your dealership a connected system for sales, operations, coaching, and growth—all in one place.</p><div className="lp-actions"><Link className="lp-button" href="/demo">See It In Action <ArrowRight/></Link><a className="lp-outline" href="#platform">Explore the Platform</a></div></div><div className="lp-product-preview"><div className="lp-mini-nav"><BrandLogo compact/><span>Home</span><span>Sales Assistant</span><span>Operations</span><span>Coaching</span><span>Reports</span></div><section><span className="lp-preview-logo"><BrandLogo compact/></span><h3>Good Morning!</h3><p>Let&apos;s make today a great day.</p><div><Link href="/assistant"><Bot/>Ask AI<small>Get instant answers</small></Link><Link href="/coach"><GraduationCap/>Role Play<small>Practice and improve</small></Link><Link href="/operations/checklists"><ClipboardCheck/>Checklists<small>Stay on track</small></Link><Link href="/executive"><BarChart3/>Reports<small>See what matters</small></Link></div></section></div></div>
      <div className="lp-hero-proof"><span><TrendingUp/>Increase Sales</span><span><UsersRound/>Stronger Teams</span><span><ClipboardCheck/>Smoother Operations</span><span><BarChart3/>Measurable Growth</span></div>
    </section>

    <section className="lp-section lp-solutions" id="solutions"><div className="lp-heading"><div><p className="lp-kicker">Built for every dealership</p><h2>More than answers.<br/>A better way to run the floor.</h2><p>RunFloor combines AI, training, process management, and data into one user-friendly platform built for dealership teams.</p></div><aside><ShieldCheck/><strong>Built for multi-location dealership teams.</strong></aside></div><div className="lp-pillar-grid">{pillars.map(({icon:Icon,title,copy,href,items},index)=><Link className={index===0?"featured":""} href={href} key={title}><Icon/><h3>{title}</h3><p>{copy}</p><ul>{items.map(item=><li key={item}><CheckCircle2/>{item}</li>)}</ul><ArrowRight/></Link>)}</div></section>
    ...
    <section className="lp-cta" id="pricing"><div><p className="lp-kicker">Ready when your team is</p><h2>Run a better <em>sales floor today.</em></h2><p>See how RunFloor can help your dealership sell smarter, coach better, and grow faster.</p></div><div><Link className="lp-button light" href="/demo"><CirclePlay/>Watch a Demo</Link><Link className="lp-button dark" href="/login">Get Started <ArrowRight/></Link></div></section>
    <footer className="lp-footer"><BrandLogo/><nav><a href="#platform">Platform</a><a href="#solutions">Solutions</a><a href="#resources">Resources</a><a href="#pricing">Pricing</a></nav><div><Link href="/demo">Demo</Link><Link href="/login">Sign in</Link></div><p>People. Processes. Profit.</p></footer>
  </main>;
}
