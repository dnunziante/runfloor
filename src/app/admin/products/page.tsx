...
export default async function AdminProductsPage({ searchParams }: { searchParams: Promise<{ family?: string; create?: string; tools?: string; q?: string; type?: string; rvType?: string; year?: string; status?: string }> }) {
  ...
  return <AppShell title="Admin · Products">
    {isRvWorkspace ? <PageHeader eyebrow="Tenant catalog" title="RV Product Models" description="Review, approve, classify, and manage RV models for this workspace." action={<Link className="btn btn-ghost" href="/admin/competitor-sources">Open Product Models</Link>}/> : <AdminProductCatalogDashboard products={result.products} families={categories} initialFamily={requestedFamilyId}/>}
