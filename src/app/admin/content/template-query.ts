...
import { TEMPLATE_PAGE_SIZE, type TemplateFilters, type TemplatePage } from "@/lib/content/template-pagination";
...
export async function getTemplatePage(filters: TemplateFilters): Promise<TemplatePage> {
  ...
  if (filters.contentType !== "email_template" && filters.contentType !== "text_template") return { ...empty, error: "Invalid template type." };
  const supabase = await createClient();
  const page = Math.max(1, Math.floor(Number(filters.page) || 1));
  let query = supabase.from("sales_content_items")
    .select("id,title,body,status,category,tags,updated_at", { count: "exact" })
    .eq("organization_id", viewer.organizationId)
    .eq("content_type", filters.contentType);
  if (filters.category && filters.category !== "All Templates") query = filters.category === "General" ? query.or("category.eq.General,category.is.null,category.eq.") : query.eq("category", filters.category);
  ...
  if (needle) {
    ...
    if (safe) query = query.or(`title.ilike.%${safe}%,body.ilike.%${safe}%,category.ilike.%${safe}%,tags.cs.{"${safe}"}`);
  }
  ...
  return { rows: (data ?? []).map((item) => ({ id: item.id, title: item.title, body: item.body, status: item.status as TemplatePage["rows"][number]["status"], category: item.category || "", tags: item.tags || [], updatedAt: item.updated_at })), total, page, error: "" };
}

export async function getTemplateFacets(contentType: "email_template" | "text_template") {
  const viewer = await getViewer();
