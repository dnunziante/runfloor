...
- Redesigned the shared Executive Trends page for the RV template and its RunFloor RV and Rayne RV workspaces. Summary cards, the approved-history panel, responsive guidance, 6/12/24-month and custom date filters, location selection, period comparison, and source detail all use tenant-scoped approved results. Missing months remain absent and no forecasts are generated.
- Redesigned the shared Monthly Leadership Review page for the RV template, RunFloor RV, and Rayne RV. The responsive meeting view retains tenant-scoped metrics, period selection, deterministic agenda links, accountability, decisions, measured outcomes, notes, printing, and review completion. Notes remain part of the completion record under the existing database model.
- Redesigned the shared Executive Accountability page for the RV template, RunFloor RV, and Rayne RV. Period selection, tenant-scoped summary counts, notice rules, reminders, owner commitments, and the append-only review trail remain connected. Added read-only notice, owner, and activity filters to the page.
- Redesigned Growth Priority Scoring around the configured factor weights, a ranked score matrix, category and sort controls, and clear paths to details, opportunities, plans, and administrator settings. The shared page applies to both RV workspaces and uses each organization's own scoring configuration.
- Redesigned Growth Action Plans with a visual overview, live plan counts, an illustrated empty state, direct opportunity and performance links, and guidance on shared versus browser-local storage. Existing plan editing and outcomes remain on the opportunity detail pages, with organization data boundaries unchanged.
- Redesigned Growth Performance with a plan-based overview, illustrated empty state, and clear paths to create plans. Existing pipeline, task progress, deadlines, and verified outcome reporting remain available when a workspace has plans; results are never inferred from task completion.
- Redesigned administrator Growth Scoring with visual weight sliders, live 100-point validation, searchable opportunity ratings, score previews, recommended-weight reset, and links to opportunity management and priority review. Saving continues to use the existing organization-scoped scoring action and browser-local demo mode.
